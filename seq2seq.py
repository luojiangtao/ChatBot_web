...
class Seq2Seq(object):
    ...
    def embedding(self):
        ...
        with tf.variable_scope('embedding'):
            encoder_embedding = tf.Variable(tf.truncated_normal(shape=[self.encoder_vocab_size, self.embedding_dim], stddev=0.1),
                                            name='encoder_embeddings')
            if not self.share_embedding:
                decoder_embedding = tf.Variable(tf.truncated_normal(shape=[self.decoder_vocab_size, self.embedding_dim], stddev=0.1),
    ...
    def build_encoder_cell(self, hidden_size, cell_type, layer_size):
        ...
        :return:
        """
        # 此处一定不能用[LSTMCell] * layer_size，否则后续会报错
